...
var createCompiledShader = function (gl, shaderType, source) {
    ...
    if (GL_DEBUG && !gl.getShaderParameter(shader, webgl_constants_1.GL.COMPILE_STATUS) && !gl.isContextLost()) {
        ...
        logger_1.Logger.warn("kinGLy exception: compilation error in a ".concat(shaderTypeName, " shader: ").concat(gl.getShaderInfoLog(shader)));
    }
    return shader;
};
...
var createLinkedProgram = function (gl, vertexShader, fragmentShader, attributeLocations) {
    ...
    if (GL_DEBUG) {
        if (!gl.getProgramParameter(program, webgl_constants_1.GL.LINK_STATUS) && !gl.isContextLost())
            logger_1.Logger.warn("kinGLy exception: shader program failed to link: ".concat(gl.getProgramInfoLog(program)));
        gl.validateProgram(program);
        if (!gl.getProgramParameter(program, webgl_constants_1.GL.LINK_STATUS) && !gl.isContextLost())
            logger_1.Logger.warn("kinGLy exception: could not validate the shader program: ".concat(gl.getProgramInfoLog(program)));
    }
    else {
...
var getUniforms = function (gl, program) {
    ...
    var uniforms = new Map(__spreadArray([], __read(new Array(gl.getProgramParameter(program, webgl_constants_1.GL.ACTIVE_UNIFORMS))), false).flatMap(function (_, index) {
        ...
        if (location === null) {
            logger_1.Logger.warn("kinGLy exception: uniform location ".concat(location, " (name: ").concat(name, ", type: ").concat(type, ") not found"));
            return [];
        }
        var setValue = location ? uniformSetterLookup[type](gl, location) : function () { };
        if (GL_DEBUG && !setValue) {
            logger_1.Logger.warn("kinGLy exception: no setValue for uniform GL[".concat(type, "] (name: ").concat(name, ") implemented yet"));
            return [];
        }
...
var createTexture = function (gl, _a) {
    ...
    if (GL_DEBUG) {
        ...
        if (error !== gl.NO_ERROR && error !== gl.CONTEXT_LOST_WEBGL)
            logger_1.Logger.warn("kinGLy exception: failed to set the texture with texImage2D, code ".concat(error));
    }
...
var getRenderer = function (gl, program, vao, _a) {
    var _b = _a.depthTest, depthTest = _b === void 0 ? false : _b, _c = _a.blend, blend = _c === void 0 ? true : _c, _d = _a.frontFace, frontFace = _d === void 0 ? webgl_constants_1.GL.CCW : _d;
    ...
    return function (_a) {
        ...
        if (uniformValues) {
            uniforms.forEach(function (setValue, name) { return uniformValues[name] && setValue(uniformValues[name]); });
        }
