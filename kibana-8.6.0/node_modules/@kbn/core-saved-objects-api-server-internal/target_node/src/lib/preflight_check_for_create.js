...
async function optionallyFindAliases(createPointInTimeFinder, objects) {
  ...
  const objectsToGetOrObjectsToFind = objects.map(object => {
    ...
    const tag = spaces.size > FIND_ALIASES_THRESHOLD || spaces.has(_coreSavedObjectsUtilsServer.ALL_NAMESPACES_STRING) ? 'Right' : 'Left';
    return {
      tag,
      value: {
        type,
  ...
  const aliasMap = await (0, _legacy_url_aliases.findLegacyUrlAliases)(createPointInTimeFinder, objectsToFind, ALIAS_SEARCH_PER_PAGE);
