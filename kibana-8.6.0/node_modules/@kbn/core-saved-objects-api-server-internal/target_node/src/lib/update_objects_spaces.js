...
async function updateObjectsSpaces({
  ...
  if (!spacesToAdd.length && !spacesToRemove.length) {
    throw _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createBadRequestError('spacesToAdd and/or spacesToRemove must be a non-empty array of strings');
  }
  if ((0, _intersection.default)(spacesToAdd, spacesToRemove).length > 0) {
    throw _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createBadRequestError('spacesToAdd and spacesToRemove cannot contain any of the same strings');
  }
  const {
    namespace
  } = options;
  ...
  const expectedBulkGetResults = objects.map(object => {
    ...
    if (!allowedTypes.includes(type)) {
      const error = errorContent(_coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createGenericNotFoundError(type, id));
      return {
        tag: 'Left',
        value: {
          id,
    ...
    if (!registry.isShareable(type)) {
      const error = errorContent(_coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createBadRequestError(`${type} doesn't support multiple namespaces`));
      return {
        tag: 'Left',
        value: {
          id,
  ...
  const expectedBulkOperationResults = expectedBulkGetResults.map(expectedBulkGetResult => {
    ...
    if (esRequestIndex !== undefined) {
      ...
      if (isErrorDoc || !(doc !== null && doc !== void 0 && doc.found) ||
      ...
      currentSpaces = (_doc$_source$namespac = (_doc$_source = doc._source) === null || _doc$_source === void 0 ? void 0 : _doc$_source.namespaces) !== null && _doc$_source$namespac !== void 0 ? _doc$_source$namespac : [];
      // @ts-expect-error MultiGetHit._source is optional
