...
function getBulkOperationError(type, id, rawResponse) {
  ...
  if (error) {
    switch (status) {
      case 404:
        return error.type === 'index_not_found_exception' ? _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createIndexAliasNotFoundError(error.index).output.payload : _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createGenericNotFoundError(type, id).output.payload;
      case 409:
        return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createConflictError(type, id).output.payload;
      default:
        return {
...
function normalizeNamespace(namespace) {
  if (namespace === _coreSavedObjectsUtilsServer.ALL_NAMESPACES_STRING) {
    throw _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createBadRequestError('"options.namespace" cannot be "*"');
