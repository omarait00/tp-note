...
function decorateEsError(error) {
  ...
  if (error instanceof ConnectionError || error instanceof NoLivingConnectionsError || error instanceof TimeoutError || responseErrors.isServiceUnavailable(error.statusCode)) {
    return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateEsUnavailableError(error, reason);
  }
  if (responseErrors.isConflict(error.statusCode)) {
    return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateConflictError(error, reason);
  }
  if (responseErrors.isNotAuthorized(error.statusCode)) {
    return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateNotAuthorizedError(error, reason);
  }
  if (responseErrors.isForbidden(error.statusCode)) {
    return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateForbiddenError(error, reason);
  }
  if (responseErrors.isRequestEntityTooLarge(error.statusCode)) {
    return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateRequestEntityTooLargeError(error, reason);
  }
  if (responseErrors.isNotFound(error.statusCode)) {
    ...
    if (match && match.length > 0) {
      return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateIndexAliasNotFoundError(error, match[1]);
    }
    ...
    if (!(0, _coreElasticsearchServerInternal.isSupportedEsServer)(error === null || error === void 0 ? void 0 : (_error$meta2 = error.meta) === null || _error$meta2 === void 0 ? void 0 : _error$meta2.headers)) {
      return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createGenericNotFoundEsUnavailableError();
    }
    return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.createGenericNotFoundError();
  }
  if (responseErrors.isTooManyRequests(error.statusCode)) {
    return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateTooManyRequestsError(error, reason);
  }
  if (responseErrors.isBadRequest(error.statusCode)) {
    if (SCRIPT_CONTEXT_DISABLED_REGEX.test(reason || '') || reason === INLINE_SCRIPTS_DISABLED_MESSAGE) {
      return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateEsCannotExecuteScriptError(error, reason);
    }
    return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateBadRequestError(error, reason);
  }
  return _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.decorateGeneralError(error, reason);
}