 ...
 */
export function validateSchema(sourceName, validator, payload) {
  // Run io-ts validation to the event
  var result = validator.decode(payload);
  either.mapLeft(result, function (validationErrors) {
    var humanFriendlyErrors = validationErrors.map(function (err) {
      var _err$message;
      return "[".concat(getFullPathKey(err.context), "]: ").concat((_err$message = err.message) !== null && _err$message !== void 0 ? _err$message : readableContext(err.context));
    }).filter(function (errMsg, idx, listOfErrMsgs) {
      return listOfErrMsgs.indexOf(errMsg, idx + 1) === -1;
    });
    throw new Error("Failed to validate payload coming from \"".concat(sourceName, "\":\n\t- ").concat(humanFriendlyErrors.join('\n\t- ')));
  });
}
...
function getFullPathKey(context) {
  ...
  .filter(function (ctx) {
    return !['InterfaceType', 'PartialType'].includes(ctx.type._tag);
  }).map(function (_ref) {
    var key = _ref.key;
    return key;
  }).filter(Boolean).join('.');
