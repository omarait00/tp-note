...
async function createOrUpgradeSavedConfig(options) {
  ...
  try {
    ...
    if (handleWriteErrors) {
      if (_coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.isConflictError(error)) {
        return;
      }
      if (_coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.isNotAuthorizedError(error) || _coreSavedObjectsUtilsServer.SavedObjectsErrorHelpers.isForbiddenError(error)) {
        return attributes;
      }
    }
    throw error;
