...
class ActionsPlugin {
  ...
  setup(core, plugins) {
    ...
    for (const preconfiguredId of Object.keys(this.actionsConfig.preconfigured)) {
      if (preconfiguredId !== _common.AlertHistoryEsIndexConnectorId) {
        ...
        this.preconfiguredActions.push({
          ...
          isDeprecated: (0, _is_connector_deprecated.isConnectorDeprecated)(rawPreconfiguredConnector)
        });
      } else {
    ...
    return {
      registerType: actionType => {
        ...
        actionType.maxAttempts = (_actionType$maxAttemp = actionType.maxAttempts) !== null && _actionType$maxAttemp !== void 0 ? _actionType$maxAttemp : _action_type_registry.MAX_ATTEMPTS;
  ...
  start(core, plugins) {
    ...
    const getUnsecuredActionsClient = () => {
      const internalSavedObjectsRepository = core.savedObjects.createInternalRepository([_saved_objects.ACTION_TASK_PARAMS_SAVED_OBJECT_TYPE]);
