...
exports.formatMlPipelineBody = exports.createIndexPipelineDefinitions = void 0;
 ...
 */
const createIndexPipelineDefinitions = async (indexName, esClient) => {
  ...
  await esClient.ingest.putPipeline({
    ...
    id: (0, _ml_inference_pipeline_utils.getInferencePipelineNameFromIndexName)(indexName),
    processors: [],
    version: 1
  });
  await esClient.ingest.putPipeline({
  ...
  await esClient.ingest.putPipeline({
    ...
    processors: [{
      ...
      pipeline: {
        ...
        name: (0, _ml_inference_pipeline_utils.getInferencePipelineNameFromIndexName)(indexName),
        on_failure: [{
          append: {
            field: '_ingestion_errors',
            value: ["Processor 'pipeline' with tag 'index_ml_inference_pipeline' in pipeline '{{ _ingest.on_failure_pipeline }}' failed with message '{{ _ingest.on_failure_message }}'"]
          }
        }],
      ...
      pipeline: {
        ...
        on_failure: [{
          append: {
            field: '_ingestion_errors',
            value: ["Processor 'pipeline' with tag 'index_custom_pipeline' in pipeline '{{ _ingest.on_failure_pipeline }}' failed with message '{{ _ingest.on_failure_message }}'"]
          }
        }],
      ...
      remove: {
        field: ['_attachment', '_attachment_indexed_chars', '_extracted_attachment', '_extract_binary_content', '_reduce_whitespace', '_run_ml_inference'],
        ignore_missing: true,
        on_failure: [{
          append: {
            field: '_ingestion_errors',
  ...
  return {
    created: [indexName, `${indexName}@custom`, (0, _ml_inference_pipeline_utils.getInferencePipelineNameFromIndexName)(indexName)]
  };
};
 ...
 */
exports.createIndexPipelineDefinitions = createIndexPipelineDefinitions;
const formatMlPipelineBody = async (pipelineName, modelId, sourceField, destinationField, esClient) => {
...
exports.formatMlPipelineBody = formatMlPipelineBody;