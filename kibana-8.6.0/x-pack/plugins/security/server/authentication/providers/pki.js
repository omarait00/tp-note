...
class PKIAuthenticationProvider extends _base.BaseAuthenticationProvider {
  ...
  async authenticate(request, state) {
    ...
    return authenticationResult.notHandled() && canStartNewSession(request) ? await this.authenticateViaPeerCertificate(request, state) : authenticationResult;
  }
   ...
   * @param [state] Optional state object associated with the provider.
   */
  async authenticateViaPeerCertificate(request, state) {
    ...
    return _authentication_result.AuthenticationResult.succeeded(this.authenticationInfoToAuthenticatedUser(result.authentication), {
      ...
      userProfileGrant: peerCertificate.fingerprint256 !== (state === null || state === void 0 ? void 0 : state.peerCertificateFingerprint256) ? {
        type: 'accessToken',
