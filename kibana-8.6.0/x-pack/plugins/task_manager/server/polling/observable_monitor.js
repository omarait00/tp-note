...
function createObservableMonitor(observableFactory, {
  ...
  return new _rxjs.Observable(subscriber => {
    const subscription = (0, _rxjs.timer)(0, heartbeatInterval).pipe(
    // switch from the heartbeat interval to the instantiated observable until it completes / errors
    (0, _operators.exhaustMap)(() => takeUntilDurationOfInactivity(observableFactory(), inactivityTimeout)),
...
function takeUntilDurationOfInactivity(source$, inactivityTimeout) {
  ...
  if (inactivityTimeout) {
    ...
    return source$.pipe((0, _operators.takeUntil)(inactivityMonitor$.pipe(
    // on each new emited value, start a new timer, discarding the old one
    (0, _operators.switchMap)(() => (0, _rxjs.timer)(inactivityTimeout)),
    ...
    (0, _operators.switchMapTo)((0, _rxjs.throwError)(new Error(`Observable Monitor: Hung Observable restarted after ${inactivityTimeout}ms of inactivity`))))),
