...
function getAssets(packageInfo, filter = path => true, datasetName) {
  ...
  for (const path of paths) {
    ...
    if (datasetName) {
      const comparePath = (packageInfo === null || packageInfo === void 0 ? void 0 : packageInfo.type) === 'input' ? `${packageInfo.name}-${packageInfo.version}/agent/input/` : `${packageInfo.name}-${packageInfo.version}/data_stream/${datasetName}/`;
