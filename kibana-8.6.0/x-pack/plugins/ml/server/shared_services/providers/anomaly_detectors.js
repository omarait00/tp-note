      ...
      return {
        async jobs(jobId) {
          return await getGuards(request, savedObjectsClient).isFullLicense().hasMlCapabilities(['canGetJobs']).ok(async ({
            ...
            const body = await mlClient.getJobs(jobId !== undefined ? {
              job_id: jobId
            } : undefined);
            return body;
          });
        ...
        async jobStats(jobId) {
          return await getGuards(request, savedObjectsClient).isFullLicense().hasMlCapabilities(['canGetJobs']).ok(async ({
            ...
            const body = await mlClient.getJobStats(jobId !== undefined ? {
              job_id: jobId
            } : undefined);
            return body;
          });
        ...
        async datafeeds(datafeedId) {
          return await getGuards(request, savedObjectsClient).isFullLicense().hasMlCapabilities(['canGetDatafeeds']).ok(async ({
            ...
            const body = await mlClient.getDatafeeds(datafeedId !== undefined ? {
        ...
        async datafeedStats(datafeedId) {
          return await getGuards(request, savedObjectsClient).isFullLicense().hasMlCapabilities(['canGetDatafeeds']).ok(async ({
            ...
            const body = await mlClient.getDatafeedStats(datafeedId !== undefined ? {
              datafeed_id: datafeedId
            } : undefined);
            return body;
          });
