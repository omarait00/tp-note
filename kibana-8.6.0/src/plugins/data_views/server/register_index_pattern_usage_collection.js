...
function registerIndexPatternsUsageCollector(getStartServices, usageCollection) {
  ...
  const indexPatternUsageCollector = usageCollection.makeUsageCollector({
    ...
    fetch: async () => {
      ...
      const indexPatternService = await dataViewsServiceFactory(new _server.SavedObjectsClient(savedObjects.createInternalRepository()), elasticsearch.client.asInternalUser);
