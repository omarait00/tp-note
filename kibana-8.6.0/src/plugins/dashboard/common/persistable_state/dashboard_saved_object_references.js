  ...
  return {
    ...
    state: {
      ...
      panels: inputPanels.reduce((current, panel, index) => {
        ...
        current[panelIndex] = (0, _dashboard_panel_converters.convertSavedDashboardPanelToPanelState)(panel);
        return current;
      }, {})
    }
  };
...
function panelStatesToPanels(panelStates, originalPanels) {
  return Object.entries(panelStates).map(([id, panelState]) => {
    ...
    return (0, _dashboard_panel_converters.convertPanelStateToSavedDashboardPanel)(panelState, (_originalPanel = originalPanel) !== null && _originalPanel !== void 0 && _originalPanel.version ? originalPanel.version : '');
  });
}
