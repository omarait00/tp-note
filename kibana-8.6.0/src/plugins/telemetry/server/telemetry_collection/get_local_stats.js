...
const getLocalStats = async (clustersDetails, config, context) => {
  ...
  return await Promise.all(clustersDetails.map(async clustersDetail => {
    const [clusterInfo, clusterStats, nodesUsage, kibana, dataTelemetry] = await Promise.all([(0, _get_cluster_info.getClusterInfo)(esClient),
    // cluster info
    (0, _get_cluster_stats.getClusterStats)(esClient),
    // cluster stats (not to be confused with cluster _state_)
    (0, _get_nodes_usage.getNodesUsage)(esClient),
    // nodes_usage info
