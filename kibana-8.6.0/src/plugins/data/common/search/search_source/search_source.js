...
var _common3 = require("../../../../expressions/common");
...
/** @internal */
const searchSourceRequiredUiSettings = ['dateFormat:tz', _2.UI_SETTINGS.COURIER_CUSTOM_REQUEST_PREFERENCE, _2.UI_SETTINGS.COURIER_IGNORE_FILTER_IF_FIELD_NOT_IN_INDEX, _2.UI_SETTINGS.COURIER_MAX_CONCURRENT_SHARD_REQUESTS, _2.UI_SETTINGS.COURIER_SET_REQUEST_PREFERENCE, _2.UI_SETTINGS.DOC_HIGHLIGHT, _2.UI_SETTINGS.META_FIELDS, _2.UI_SETTINGS.QUERY_ALLOW_LEADING_WILDCARDS, _2.UI_SETTINGS.QUERY_STRING_OPTIONS, _2.UI_SETTINGS.SEARCH_INCLUDE_FROZEN, _2.UI_SETTINGS.SORT_OPTIONS];
...
class SearchSource {
    ...
    const activeIndexPattern = (_filters = filters) === null || _filters === void 0 ? void 0 : _filters.reduce((acc, f) => {
      if (f.meta.key === '_index' && f.meta.disabled === false) {
        if (f.meta.negate === false) {
          var _f$meta$params$query;
          return _lodash.default.concat(acc, (_f$meta$params$query = f.meta.params.query) !== null && _f$meta$params$query !== void 0 ? _f$meta$params$query : f.meta.params);
        } else {
          if (Array.isArray(f.meta.params)) {
            return _lodash.default.difference(acc, f.meta.params);
          } else {
            return _lodash.default.difference(acc, [f.meta.params.query]);
          }
        }
      } else {
        return acc;
  ...
  hasPostFlightRequests() {
    ...
    if (aggs instanceof _2.AggConfigs) {
    ...
    if (aggs instanceof _2.AggConfigs) {
      for (const agg of aggs.aggs) {
        if (agg.enabled && typeof agg.type.postFlightRequest === 'function') {
          ...
          response = await agg.type.postFlightRequest(response, aggs, agg, this, (_options$inspector = options.inspector) === null || _options$inspector === void 0 ? void 0 : _options$inspector.adapter, options.abortSignal, options.sessionId, options.disableShardFailureWarning);
        }
      }
    }
    return response;
  ...
  fetchSearch$(searchRequest, options) {
    ...
    return search({
      ...
      return new _rxjs.Observable(obs => {
        if ((0, _2.isErrorResponse)(response)) {
          obs.error(response);
        } else if ((0, _2.isPartialResponse)(response)) {
      ...
      if (!(0, _2.isCompleteResponse)(response)) {
        return response;
      }
  ...
  mergeProp(data, val, key) {
    ...
    switch (key) {
      ...
      case 'sort':
        const sort = (0, _normalize_sort_request.normalizeSortRequest)(val, this.getField('index'), getConfig(_2.UI_SETTINGS.SORT_OPTIONS));
        return addToBody(key, sort);
      case 'pit':
        return addToRoot(key, val);
      case 'aggs':
        if (val instanceof _2.AggConfigs) {
  ...
  getFieldsWithoutSourceFilters(index, bodyFields) {
    ...
    const filter = (0, _common.fieldWildcardFilter)(sourceFiltersValues, this.dependencies.getConfig(_2.UI_SETTINGS.META_FIELDS));
  ...
  flatten() {
    ...
    const metaFields = (_getConfig = getConfig(_2.UI_SETTINGS.META_FIELDS)) !== null && _getConfig !== void 0 ? _getConfig : [];
    ...
    if (index) {
      ...
      const filter = (0, _common.fieldWildcardFilter)(body._source.excludes, metaFields);
    ...
    if (highlightAll && body.query) {
      body.highlight = (0, _common2.getHighlightRequest)(getConfig(_2.UI_SETTINGS.DOC_HIGHLIGHT));
   ...
   */
  getSerializedFields(recurse = false, includeFields = true) {
    const {
      filter: originalFilters,
    ...
    if (index) {
      serializedSearchSourceFields.index = index.isPersisted() ? index.id : index.toSpec(includeFields);
    }
    if (sort) {
  ...
  toExpressionAst({
    ...
    const aggConfigs = aggs instanceof _2.AggConfigs ? aggs : index && aggs && this.dependencies.aggs.createAggConfigs(index, aggs);
    if (aggConfigs) {
